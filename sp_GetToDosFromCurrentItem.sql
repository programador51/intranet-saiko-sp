...
-- Description: Gets the ToDos for a espesific item 
-- STORED PROCEDURE NAME:	sp_GetToDosFromCurrentItem
-- **************************************************************************************************************************************************
-- =============================================
-- PARAMETERS:
-- @executiveId: Executive Id
-- @fromId: The iemt id, could be the documentId, contactId or customerId
-- @type: Identifys the fromId 
-- @taskFinished: Indicates if the process is finished or not
-- ===================================================================================================================================
-- =============================================
-- VARIABLES:
-- ===================================================================================================================================
...
-- Create Date: 06/17/2022
-- Description: sp_GetToDosFromCurrentItem - Gets the ToDos for a espesific item 
CREATE PROCEDURE sp_GetToDosFromCurrentItem(
    @executiveId INT,
    @fromId INT,
    @type INT,
    @taskFinished TINYINT
) AS 
BEGIN
    ...
    SELECT 
        ...
        ToDo.atentionDate AS attentionDate,
        ToDo.fromId,
    ...
    WHERE 
        ...
        ToDo.fromId= @fromId AND 
        ToDo.idSection= @type AND
        ToDo.isTaskFinished =@taskFinished
    ORDER BY dbo.fn_MexicoLocalTime(atentionDate) ASC
