...
ALTER PROCEDURE [dbo].[sp_RobotContractRenewal] AS 
BEGIN
...
--? ----------------- ↓↓↓ INSERCION DE TODAS LAS TABLAS TEMPORALES  ↓↓↓ -----------------------
    ...
    -- + ----------------- ↓↓↓ INSERCIÓN DE LAS COTIZACIONES TEMPORALES (CONTRATOS) ↓↓↓ -----------------------
        ...
        INSERT INTO #TempCotizaciones (
            ...
            WHERE 
                ...
                contractDocument.idStatus=7 AND 
    ...
    -- + ----------------- ↓↓↓ ACTUALIZAR LA MONEDA DEL DOCUMENTO A MXN SEGUN EL COUNTMXN ↓↓↓ -----------------------
        UPDATE #TempCotizaciones SET 
            idCurrency= 1
        WHERE (countMXN >0 AND countUSD<1)
        ...
        SELECT 
            ...
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND((catalogo.unit_price*quantity),2) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND((catalogo.unit_price*quantity)*@currentTc,2))
                ELSE
                    (ROUND((catalogo.unit_price*quantity)/@currentTc,2))
            ...
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND((catalogo.unit_price*quantity*catalogo.iva/100),2) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND(((catalogo.unit_price*quantity*catalogo.iva/100)*@currentTc),2) )
                ELSE
                    (ROUND(((catalogo.unit_price*quantity*catalogo.iva/100)/@currentTc),2) )
            END,--IVA
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND(((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100)),2))
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND((((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100))* @currentTc),2) )
                ELSE
                    (ROUND((((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100))/ @currentTc) ,2))
            END,--  SUB_TOTAL
            @0,--PRECIO_UNITARIO_VENTA
            catalogo.unit_price, --@0,--PRECIO_UNITARIO_ANTES_CAMBIO
            catalogo.unit_cost,--@0,--COSTO_UNITARIO_ANTES_CAMBIO
            ROUND((catalogo.unit_price*quantity*catalogo.iva/100),2),--@0,--IVA_ANTES_CAMBIO
            (ROUND(((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100)),2)),--@0,--SUB_TOTAL_ANTES_CAMBIO
            catalogo.unit_price,--@0,--PRECIO_UNITARIO_VENTA_ANTES_CAMBIO
            @0,--CALCULAR_COSTO_DESCUENTO
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND((catalogo.unit_cost*quantity),1) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND(((catalogo.unit_cost*quantity)*@currentTc),2))
                ELSE
                    (ROUND(((catalogo.unit_cost*quantity)/@currentTc),2))
            END,--CALCULAR_IMPORTE_COSTO
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND((catalogo.unit_cost*quantity*catalogo.iva/100),2) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND(((catalogo.unit_cost*quantity*catalogo.iva/100)*@currentTc),2) )
                ELSE
                    (ROUND(((catalogo.unit_cost*quantity*catalogo.iva/100)/@currentTc),2) )
            ...
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100),2) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND((((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100))* @currentTc),2) )
                ELSE
                    (ROUND((((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100))/ @currentTc) ,2))
            ...
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND(((catalogo.unit_price*quantity) + (catalogo.unit_price*quantity*catalogo.iva/100)),2) )
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND((((catalogo.unit_price*quantity) + (catalogo.unit_price*quantity*catalogo.iva/100))*@currentTc),2))
                ELSE
                    (ROUND((((catalogo.unit_price*quantity) + (catalogo.unit_price*quantity*catalogo.iva/100))/@currentTc),2))
            ...
            CASE 
                WHEN tempCotizacion.idCurrency= catalogo.currency THEN 
                    (ROUND(((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100)),2) - ROUND(((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100)),2))
                WHEN tempCotizacion.idCurrency=1 AND catalogo.currency=2 THEN 
                    (ROUND((((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100))* @currentTc),2) - ROUND((((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100))* @currentTc),2))
                ELSE
                    (ROUND((((catalogo.unit_price*docItems.quantity)+(catalogo.unit_price*docItems.quantity*catalogo.iva/100))/ @currentTc) ,2) - ROUND((((catalogo.unit_cost*docItems.quantity)+(catalogo.unit_cost*docItems.quantity*catalogo.iva/100))/ @currentTc) ,2))
            END-- UTILIDADES
    ...
    UPDATE tempDocument SET
        totalAmount = (SELECT SUM(items.totalImport) FROM #TemDocumentItems AS items  WHERE items.currentDocumentId= tempDocument.currentDocumentId),
        subTotalAmount= (SELECT SUM(items.subTotal) FROM #TemDocumentItems AS items  WHERE items.currentDocumentId= tempDocument.currentDocumentId) ,
        ivaAmount=(SELECT SUM(items.iva) FROM #TemDocumentItems AS items WHERE items.currentDocumentId= tempDocument.currentDocumentId)
    FROM #TempCotizaciones AS tempDocument
    LEFT JOIN #TemDocumentItems AS tempItems ON tempItems.currentDocumentId=tempDocument.currentDocumentId
    WHERE tempDocument.currentDocumentId = tempItems.currentDocumentId
...
--* ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
/*
    Se tiene que agregar la relacion del contrato con la cotización

*/
--? ----------------- ↓↓↓  INSERCION DE LA RELACION DEL CONTRATO CON LA NUEVA COTIZACIÓN ↓↓↓ -----------------------
    INSERT INTO ContractQuotes (
        idContract,
        idQuote,
        reminderDate,
        expirationDate,
