...
-- Description: 
-- STORED PROCEDURE NAME:	sp_GetOdcFromProyects
-- **************************************************************************************************************************************************
-- =============================================
-- PARAMETERS:
-- @idProyect INT - Id proyect to filter
...
-- *****************************************************************************************************************************
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name ='sp_GetOdcFromProyects')
    BEGIN 

        DROP PROCEDURE sp_GetOdcFromProyects;
    END
GO
SET ANSI_NULLS ON
GO
...
-- Create Date: 07/18/2024
-- Description: sp_GetOdcFromProyects - Some Notes
CREATE PROCEDURE sp_GetOdcFromProyects(
    @idProyect INT,
    @limit INT,
    @page INT,
    @orderBy NVARCHAR(4)
...
BEGIN
    ...
    SELECT 
        ...
        (
            SELECT 
                CASE 
                    WHEN SUM(items.quantity) - SUM(items.receivedMaterials) = 0 THEN 'Recibidos'
                    WHEN (SUM(items.receivedMaterials)>0) AND (SUM(items.quantity) - SUM(items.receivedMaterials) > 0) THEN 'Por recibir'
                    ELSE 'No recibidos'
                END
